...
from gps_imu.srv import convert_to_quaternion
from gps_imu.msg import Vectornav


def readimu():
    rospy.init_node("imu_Publisher", anonymous=True)
    port = rospy.get_param('~/imu_Publisher/imu_port')
    #print(port)
    #port = '/dev/ttyUSB0'
    ...
    while not rospy.is_shutdown():
        ...
        if "VNYMR" in line:
            ...
            #msg.header.stamp.secs = float(time.to_sec)
            #msg.header.stamp.nsecs = float(time.to_nsec)
            ...
            #print("accle_x =", accx, "accle_y = ", accy, "accle_z = ", accz)
            pub.publish(msg)
